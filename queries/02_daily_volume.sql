-- Clober V2 Daily Volume (Base)
-- Daily trading volume in USD
-- Visualization: Bar chart (x: day, y: volume_usd)
...
trades AS (
    SELECT
        ...
        CAST(t.unit AS DOUBLE) * CAST(b.unit_size AS DOUBLE)
            / pow(10, COALESCE(qt.decimals, 18)) AS quote_amount,
...
trades_usd AS (
    SELECT
        tr.block_date,
        tr.quote_amount * COALESCE(p.price, 0) AS volume_usd
    FROM trades tr
